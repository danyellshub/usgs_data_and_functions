...
## Extracting Sen's slope

```{r}

custom_sens <- function(df){
  sens_test <- sens.slope(df$q_base)
  return(sens_test)
}
...
bq_trends_slopes <- bq_trends_mk %>%
  ...
  mutate(sens_tidy = map(sens_slope, broom::tidy))

unnested_bq <- bq_trends_slopes%>%
  unnest(sens_tidy)
